...
- **クラシック論文は2010年以降に限定する（確定・2026-08-15）。** 当初1980〜1990年代の論文（H.ピロリ発見など）を投入していたが、本人から「さすがに古すぎる」とフィードバックがあり、2010年以降の論文に絞る方針に変更した。
- クラシック論文の候補は、新着検索とは別に「クラシック論文ライブラリ」として静的にキュレーションする（`高impポスト.md`の医学論文版のようなイメージ）。2026-08-15時点でデモに投入した実例（いずれもPubMedで書誌情報を確認済み、すべてNEJM掲載）：
  - van Nood E, et al. N Engl J Med 2013（PMID 23323867）— 便移植（FMT）による再発性C.ディフィシル感染症治療。効果が大きすぎ中間解析で早期中止となった代表的トライアル。
  - Feagan BG, et al. N Engl J Med 2013（PMID 23964932）— vedolizumab（腸選択的生物学的製剤）のUC導入・維持療法（GEMINI 1試験）
  - Afdhal N, et al. N Engl J Med 2014（PMID 24725239）— レジパスビル・ソホスブビル併用によるHCV治療（ION-1試験）。C型肝炎をインターフェロン不要の経口薬で治せる時代を切り拓いた。
  - Bretthauer M, et al. N Engl J Med 2022（PMID 36214590）— 大腸内視鏡スクリーニングの効果を検証した初の大規模RCT（NordICC試験）。事前の期待より小さい効果で大きな議論を呼んだ。
- ライブラリは今後随時追加していく想定（消化器内科の他の2010年以降のlandmark trial／practice-changing paperを継続的に拾う。年代の下限は今後さらに調整の余地あり）。
...
3. **表示**：自己完結型HTML（1ファイル、外部依存なし）を生成し、**Artifact機能で公開**（`claude.ai/code/artifact/...` の固定URL、スマホから閲覧可）。同じファイルパスで再公開すればURLは変わらず更新できる。クラシック4本＋最新5本を全てページ内に埋め込み、「表示設定」パネル（表示件数・クラシック比率）でクライアント側フィルタする方式。
   - デモ公開済み：https://claude.ai/code/artifact/7cf744ff-ecee-4c90-b857-8dcdd99cbb81 （2026-08-15時点で、2010年以降の実在のクラシック論文4本（FMT・HCV経口薬・vedolizumab・NordICC）＋NEJM/JAMA/Lancet Gastroenterol Hepatolの最新論文5本を実際に選定・翻訳。結論ファーストのカードデザイン＋比率調整可能な表示設定パネル）
...
- [ ] クラシック論文ライブラリの拡充（現状デモ用に2010年以降の4本のみ。継続的にlandmark論文を追加していく運用の設計）
